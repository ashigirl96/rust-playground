...
mod test {
    ...
    fn sort_str_ascending() {
        let mut x = vec!["Rust", "is", "fast", "and", "memory", "with", "no", "GC"];
        assert_eq!(sort(&mut x, &Ascending), Ok(()));
        assert_eq!(
            x,
            vec!["GC", "Rust", "and", "fast", "is", "memory", "no", "with"]
        )
    }
    ...
    fn sort_str_descending() {
        let mut x = vec!["Rust", "is", "fast", "and", "memory", "with", "no", "GC"];
        let mut result = vec!["GC", "Rust", "and", "fast", "is", "memory", "no", "with"];
        result.reverse();
        assert_eq!(sort(&mut x, &Descending), Ok(()));
        assert_eq!(x, result);

        println!("is_ok: {0:?}", sort(&mut x, &Descending).is_ok())
    }
}
