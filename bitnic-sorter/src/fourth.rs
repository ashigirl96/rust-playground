    ...
    F: Sync + Fn(&T, &T) -> Ordering,
{
    if x.len() > 1 {
        ...
        if mid_point > PARALLEL_THRESHOLD {
            rayon::join(
                || sub_sort(former_x, true, comparator),
                || sub_sort(paster_x, false, comparator),
            );
        } else {
            sub_sort(former_x, forward, comparator);
            sub_sort(paster_x, forward, comparator);
        }
    }
}
...
mod test {
    ...
    #[test]
    fn sort_u32() {
        let mut x = new_u32_vec(2usize.pow(12));
        assert_eq!(sort(&mut x, &Ascending), Ok(()));
